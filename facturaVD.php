...
if ($row)
{
	...
	{
		mysql_close($link);
		mover("crearFactura2.php","El Pedido ya fue Facturado");
	}
	if($estado=='A')
	{
		mysql_close($link);
		mover("crearFactura2.php","El Pedido está Anulado");
	}
	if($estado=='P')
	{
		mysql_close($link);
		...
		echo '<form method="post" action="inv_pedVD.php" name="form3">'; 
...
{
	mysql_close($link);
	mover("crearFactura2.php","No existe la Orden de Pedido");
}
...
?>
     <form method="post" action="make_facturaVD.php" name="form1">	
    <tr>
    ...
    <tr>
        ...
        <td><?php echo $row['tipo_precio']; ?><input type="hidden" name="tip_prec" value="<?php echo $row['Id_precio'];?>">
        <input name="ord_comp" type="hidden" size=20 value="0">
        <input name="descuento" type="hidden" size=20 value="25"></td>
    </tr>
    <tr>
