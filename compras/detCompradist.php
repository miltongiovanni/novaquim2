...
{	
	$link=conectarServidor();
	$qry="select sum(Cantidad*Precio) as Total, sum(Cantidad*Precio*tasa) as IVA, tasa_retica
	from det_compras, distribucion, tasa_iva, compras, proveedores, tasa_reteica
	where det_compras.Id_compra=$Factura AND Codigo=distribucion.Id_distribucion AND tasa_iva.Id_tasa=distribucion.Cod_iva and compras.Id_compra=det_compras.Id_compra and nit_prov=NIT_provee and numtasa_rica=Id_tasa_retica";
	$result=mysqli_query($link,$qry);
	$row=mysqli_fetch_array($result);
	$qryc="select ret_provee, regimen_provee from compras, proveedores where nit_prov=NIT_provee and Id_compra=$Factura";
...
<form method="post" action="detCompradist.php" name="form1">
  <table border="0"  align="center" summary="encabezado" width="74%" cellpadding="0" >
    ...
    <tr>
      ...
      <td colspan="2"><div align="left"><?php echo '$ <script language="javascript" type="text/javascript"> document.write(commaSplit('.$row['total_fact'].'))</script>' ;?> </div></td>
    </tr>
    <tr>
    <td width="28%" colspan="1"><div align="right"><strong>NIT</strong></div></td>
      ...
      <td colspan="2"><div align="left"><?php echo '$ <script language="javascript" type="text/javascript"> document.write(commaSplit('.$row['ret_ica'].'))</script>' ;?> </div></td>
     </tr>
    <tr>
      ...
      <td colspan="2"><div align="left"><?php echo '$ <script language="javascript" type="text/javascript"> document.write(commaSplit('.$row['retencion'].'))</script>' ;?> </div></td>
    </tr>
    <tr>
      ...
      <td colspan="2"><div align="left"><?php echo '$ <script language="javascript" type="text/javascript"> document.write(commaSplit('.($row['total_fact']-$row['retencion']-$row['ret_ica']).'))</script>' ;?> </div></td>
    </tr>
    <tr>
      <td colspan="7">&nbsp;</td>
    </tr>
      ...
      <td colspan="7"><hr></td>
          ...
          <?php
			...
			echo '</td><td><div align="center">'.$row['Codigo'].'</div></td>
			  ...
			  <td><div align="center">$ <script language="javascript" type="text/javascript"> document.write(commaSplit('.$row['Precio'].'))</script></div></td><td>';
			  if ($estadoc!=7)
				{
