...
// Rename sheet
$spreadsheet->getActiveSheet()->setTitle('Inventario MP');

$invMPrimasOperador = new InvMPrimasOperaciones();
$invMPrimas = $invMPrimasOperador->getTableDetalleInvMPrima();
...
foreach ($invMPrimas as $invMPrima)
{
	...
	$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(3, $j, $invMPrima['loteMP']);
	$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(4, $j, $invMPrima['invMP']);
	$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(5, $j++, $invMPrima['precioMPrima']);
}
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$spreadsheet->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Xlsx)
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Inv_MP.xlsx"');
