...
<body>
<div id="contenedor">
<div id="saludo1"><strong>LISTA DE &Oacute;RDENES DE PEDIDO POR CLIENTE</strong></div> 
...
$sql="	select Id_pedido, Fech_pedido, Fech_entrega, tipo_precio, Nom_clien, pedido.Estado, Nom_sucursal, Dir_sucursal from pedido, tip_precio, clientes, clientes_sucursal where Nit_cliente=clientes.Nit_clien and clientes_sucursal.Nit_clien=clientes.Nit_clien and clientes.Nit_clien='$cliente' and tip_precio=Id_precio and Id_sucurs=Id_sucursal order by Id_pedido DESC;";
...
if ($total_paginas > 1){ 
   	for ($i=1;$i<=$total_paginas;$i++){ 
      	 if ($pagina == $i) 
         	 ...
         	 echo "<a href='listarPedidoClien.php?pagina=" . $i . "&cliente=" . $cliente . "'>" . $i . "</a>&nbsp;"; 
   	} 
}
echo '</div>';
...
$ssql = "select Id_pedido, Fech_pedido, Fech_entrega, tipo_precio, Nom_clien, pedido.Estado, Nom_sucursal, Dir_sucursal from pedido, tip_precio, clientes, clientes_sucursal where Nit_cliente=clientes.Nit_clien and clientes_sucursal.Nit_clien=clientes.Nit_clien and clientes.Nit_clien='$cliente' and tip_precio=Id_precio and Id_sucurs=Id_sucursal  and Id_cat_clien<>13 order by Id_pedido DESC limit " . $inicio . "," . $TAMANO_PAGINA; 
