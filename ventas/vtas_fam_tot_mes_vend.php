...
$year = $_POST['year'];
$type = $_POST['type'];
$operadorPersonal = new PersonalOperaciones();
$personal = $operadorPersonal->getPerson($idPersonal);
...
<body>
<div id="contenedor">
    <div id="saludo1"><strong>CONSULTA DE <?= $type == 1 ? 'UNIDADES' : 'VALOR' ?> POR VENTAS POR FAMILIA DE PRODUCTOS
            POR MES AÑO <?= $year ?> VENDEDOR <?= $personal['nomPersonal']; ?></strong></div>
    ...
    <table width="100%" align="center" border="0">
        <tr>
            <form action="vtas_fam_tot_mes_vend_Xls.php" method="post" target="_blank">
                ...
                <input name="FchIni" type="hidden" value="<?php echo $FchIni ?>"><input name="FchFin" type="hidden"
                                                                                        value="<?php echo $FchFin ?>"><input
                        name="vendedor" type="hidden" value="<?php echo $vendedor ?>"></form>
            <td width="9%">
...
from det_factura, factura, prodpre, clientes where idFactura=idFactura and  fechaFactura>='$FchIni' and fechaFactura<='$FchFin' and Cod_prese=codProducto and month(fechaFactura)=$b and Cod_ant=$codigo_ant and Nit_cliente=nitCliente and codVendedor=$vendedor;";
//echo $sqlv."<br>";
