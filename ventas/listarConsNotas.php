...
if(isset($_GET['FchIni'])) 
{
    $FchIni = $_GET['FchIni']; 
}	
if(isset($_GET['FchFin'])) 
{
    $FchFin = $_GET['FchFin']; 
}		
...
$sql="	select idNotaC, nomCliente, fechaNotaC, facturaOrigen, facturaDestino, Motivo, totalNotaC from nota_c, clientes where Nit_cliente=nitCliente and fechaNotaC>='$FchIni' and fechaNotaC<='$FchFin' order by idNotaC DESC;";
...
if ($total_paginas > 1){ 
   	for ($i=1;$i<=$total_paginas;$i++){ 
      	 if ($pagina == $i) 
         	 ...
         	 echo '<a href="'.'listarConsNotas.php?pagina='. $i .'&FchIni='.$FchIni.'&FchFin='.$FchFin.'">' . $i . '</a>&nbsp;'; 
   	} 
}
echo '</div>';

$ssql="	select idNotaC, nomCliente, fechaNotaC, facturaOrigen, facturaDestino, Motivo, totalNotaC from nota_c, clientes where Nit_cliente=nitCliente and fechaNotaC>='$FchIni' and fechaNotaC<='$FchFin' order by idNotaC DESC limit " . $inicio . "," . $TAMANO_PAGINA;
