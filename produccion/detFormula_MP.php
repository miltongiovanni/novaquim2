...
if($CrearFormula==0)
{
   ...
   $qryForm="insert into formula_mp (Cod_mp) values ($mprima1)";
   if($resultfact=mysqli_query($link,$qryForm))
   {
		$qry="select max(Id_form_mp) as Form from formula_mp";
		$result=mysqli_query($link,$qry);
		$row=mysqli_fetch_array($result);
		$Formula=$row['Form'];
		echo '<form method="post" action="detFormula_MP.php" name="form3">';
...
?>
 <?php
	  	$link=conectarServidor();
	  	$qry="select Cod_mp, Nom_mprima from formula_mp, mprimas where Cod_mp=Cod_mprima and Id_form_mp=$Formula";
		$result=mysqli_query($link,$qry);
		$row=mysqli_fetch_array($result);
		mysqli_free_result($result);
/* cerrar la conexión */
mysqli_close($link);
	 ?>  
<div id="contenedor">
<div id="saludo1"><strong>INGRESO DEL DETALLE DE FÓRMULA DE
...
<?php
$link=conectarServidor();
$qry="select det_formula_mp.Cod_mprima as codigo, Nom_mprima, porcentaje from mprimas, det_formula_mp 
where Id_formula_mp=$Formula and det_formula_mp.Cod_mprima=mprimas.Cod_mprima";
$result=mysqli_query($link,$qry);
$n=0;
while($row=mysqli_fetch_array($result))
{
	...
	echo'<tr>
				...
				<td align="center" ><form action="updateFormula_mp.php" method="post" name="actualiza"><input type="submit" name="Submit" value="Cambiar" class="formatoBoton">
		...
		<td align="center"><form action="delmapForm_mp.php" method="post" name="elimina"><input type="submit" name="Submit" value="Eliminar" class="formatoBoton">
				<input name="IdForm" type="hidden" value="'.$Formula.'">
				<input name="mprima" type="hidden" value="'.$codmp.'">
				<input name="percent" type="hidden" value="'.$percent.'">
			</form></td>
