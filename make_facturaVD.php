...
if(($dias_v>=0)&&($dias_f>=0))
{
	...
	$resultr=mysql_db_query($bd,$qryr);
	$qrylot="select max(Id_remision) as rem from remision";
	$resultlot=mysql_db_query($bd,$qrylot);
	$rowlot=mysql_fetch_array($resultlot);
	$Id_Rem=$rowlot['rem'];		
	/*CREACIÓN DEL ENCABEZADO DE LA FACTURA*/
	$qry="insert into factura (Factura, Nit_cliente, Fech_fact, Fech_venc, tip_precio, Ord_compra, Id_pedido, Descuento, Estado, Id_remision, Observaciones) values ($factura, '$nit','$FchVta','$FchVen', $tip_prec, $ord_comp, $pedido, $descuento/100, 'P', $Id_Rem, '$observa')";
	...
	union
    select Id_ped, Cod_producto as Producto, Can_producto as Cantidad, Prec_producto as Precio, tasa 
	from det_pedido, herramientas, tasa_iva where Id_ped=$pedido and Cod_producto>=1000000 and Cod_producto=Id_herramienta and Cod_iva=Id_tasa;
	;";
	$resultped=mysql_db_query($bd,$qryped);
	while($rowped=mysql_fetch_array($resultped))
	{
		...
		$resultdet=mysql_db_query($bd,$qrydet);
	}
	...
	$total= $subtotal-$descuento+$iva10+$iva16;
		...
		mysql_close($link);
		echo '<form action="det_facturaVD.php" method="post" name="formulario">';
		echo '<input name="factura" type="hidden" value="'.$factura.'"/><input type="submit" name="Submit" value="Cambiar" />';
		echo '</form>'; 
